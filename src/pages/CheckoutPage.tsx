...
import dayjs from 'dayjs';
import { toast } from 'sonner';
import { cn } from '@/lib/utils'; // Import utility cn
...
// --- STYLING CONSTANTS ---
// Container responsif: max-width tetap di desktop, width 100% di mobile
const CONTAINER_STYLE = {
  maxWidth: '1002px',
  width: '100%',
  marginTop: '48px',
...
// Font styles
const DISPLAY_SM_BOLD = {  fontWeight: 700, fontSize: '1.2rem', lineHeight: '2.375rem', letterSpacing: '-0.02em', color: '#0A0D12' };
const TEXT_SM_BOLD = { fontWeight: 700, fontSize: '0.875rem', lineHeight: '1.25rem', letterSpacing: '-0.02em', color: '#0A0D12' };
const TEXT_MD_SEMIBOLD = { fontWeight: 600, fontSize: '1rem', lineHeight: '1.5rem', letterSpacing: '-0.02em', color: '#0A0D12' };
const TEXT_MD_BOLD = { fontWeight: 700, fontSize: '1rem', lineHeight: '1.5rem', letterSpacing: '-0.02em', color: '#0A0D12' };
  ...
  return (
    <>
      ...
      <div className="flex flex-col px-4 pb-12 min-h-screen" style={CONTAINER_STYLE}>
        ...
        {/* Title Responsif */}
        <h1 className="text-left font-bold text-3xl md:text-4xl text-[#0A0D12]">Checkout</h1>
        ...
        <div className="flex flex-col lg:flex-row gap-8 relative">
          ...
          {/* KOLOM KIRI */}
          {/* min-w-0 penting agar flex item tidak overflow container parent */}
          <div className="flex-1 flex flex-col gap-8 min-w-0">
            ...
            <div className="space-y-4">
              ...
              <div className=" space-y-4">
                  <div className=" gap-4">
                      <div className="flex justify-between items-center w-full mb-2">
                          <p className="text-sm text-muted-foreground whitespace-nowrap mr-4">Name</p>
                          <p className="font-medium text-[#0A0D12] text-right truncate">
                            {userData?.profile.name || '-'}
                          </p>
                      </div>
                      <div className="flex justify-between items-center w-full mb-2">
                          <p className="text-sm text-muted-foreground whitespace-nowrap mr-4">Email</p>
                          <p className="font-medium text-[#0A0D12] text-right truncate">
                            {userData?.profile.email || '-'}
                          </p>
                      </div>
                      <div className="flex justify-between items-center w-full">
                          <p className="text-sm text-muted-foreground text-left whitespace-nowrap mr-4">Nomor Handphone</p>
                          <p className="font-medium text-[#0A0D12] text-right">-</p>
                      </div>
                  </div>
              </div>
                  ...
                  {itemsToCheckout.map((item) => (
                      <div key={item.id} className="flex items-start gap-4">
                          ...
                          <div className="flex flex-col justify-center space-y-1 text-left min-w-0">
                              <p className="text-xs text-muted-foreground border w-fit rounded-md mx-0.5 p-1 font-medium uppercase tracking-wide truncate">
          ...
          <div className="shrink-0 w-full lg:w-auto">
              <Card 
                  // w-full untuk mobile, fixed width di desktop
                  className={cn(
                    "flex flex-col bg-white shadow-sm",
                    "lg:sticky lg:top-24", // Sticky hanya di desktop
                    "w-full lg:w-[478px]"  // Width responsif
                  )}
                  style={{ minHeight: '640px', borderRadius: '20px', padding: '20px', gap: '24px' }}
              >
                  <h2 style={DISPLAY_SM_BOLD}  >Complete Your Borrow Request</h2>
                  ...
                  <div className="space-y-1 text-left ">
                      <p style={TEXT_SM_BOLD}>Borrow Date</p>
                      <div className="flex items-center gap-2 bg-gray-50 p-3 rounded-lg border border-gray-100 justify-between">
                          <span style={TEXT_MD_SEMIBOLD}>{today.format('DD MMM YYYY')}</span>
                          <Calendar className="w-5 h-5 text-gray-500 " />
                      </div>
                  </div>
                  ...
                  <div className="space-y-3 text-left">
                      ...
                      <div className="flex flex-col gap-3">
                          {DURATION_OPTIONS.map((days) => (
                              <label key={days} className={`flex items-center p-3 rounded-xl  cursor-pointer transition-all ${duration === days ? ' border-gray-200 ' : ' border-[#1C65DA] '}`}>
                                  <input 
                                      type="radio" name="duration" value={days} 
                                      checked={duration === days} onChange={() => setDuration(days)}
                                      className="w-5 h-5 text-[#1C65DA] border-[#1C65DA] focus:ring-gray-300"
                                  />
                                  <span className="ml-3 font-medium text-gray-700">{days} days</span>
                              </label>
                          ))}
                  ...
                  <div className="space-y-3 pt-2">
                      <div className="flex items-start space-x-3">
                          <input 
                              type="checkbox" id="agree-return"
                              checked={agreedReturn} onChange={(e) => setAgreedReturn(e.target.checked)}
                              className="mt-1 w-4 h-4 rounded border-gray-300 text-[#1C65DA] focus:ring-[#1C65DA] shrink-0"
                          />
                          <Label htmlFor="agree-return" className="text-sm leading-tight font-normal text-gray-600 cursor-pointer">
                              I agree to return the book(s) before the due date.
                          </Label>
