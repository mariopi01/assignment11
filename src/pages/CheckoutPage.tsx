...
//     title: string;
//     coverImage: string | null;
//     // Update tipe agar bisa menerima data object lengkap atau partial
//     author?: { id?: number; name: string }; 
//     category?: { id?: number; name: string };
//   };
// }

// // Interface untuk State React Router
...
// // --- STYLING CONSTANTS ---
// // (Biarkan styling constants sama seperti sebelumnya)
// const CONTAINER_STYLE = {
//   maxWidth: '1002px',
//   width: '100%',
//   marginTop: '48px',
...
// export default function CheckoutPage() {
//   const navigate = useNavigate();
//   const location = useLocation(); // Hook untuk akses state
//   const user = useAppSelector(selectCurrentUser);
//   const queryClient = useQueryClient();

//   // 1. Ambil data items dari state navigasi (jika ada)
...
//   // Hanya dijalankan jika TIDAK ada data dari state (misal user refresh halaman)
...
//       return res.data.data;
//     },
//     enabled: !stateItems, // Disable query jika data state sudah ada
//   });

//   // Tentukan sumber data items: State (Prioritas) -> API (Fallback) -> Kosong
//   const itemsToCheckout = stateItems || cartData?.items || [];
  
//   // Jika menggunakan fallback API (isCartPending true) maka loading
//   const isLoading = !stateItems && isCartPending;
...
//   // Redirect jika data kosong (opsional, untuk UX yang lebih baik)
//   useEffect(() => {
//     if (!isLoading && itemsToCheckout.length === 0 && !isCartError) {
//        // Bisa redirect kembali ke cart atau tampilkan pesan kosong
//        // navigate('/cart'); 
//     }
//   }, [isLoading, itemsToCheckout, isCartError, navigate]);
...
//                         {/* Detail Buku */}
//                         <div className="flex flex-col justify-center space-y-1 text-left">
//                             {/* Tampilkan Kategori dari data yang dioper */}
//                             <p className="text-xs text-muted-foreground font-medium uppercase tracking-wide">
...
//                                 {item.book.title}
//                             </h3>
//                             {/* Tampilkan Author dari data yang dioper */}
//                             <p className="text-sm text-gray-600">
//                                 {item.book.author?.name || 'Unknown Author'}
//                             </p>
//                         </div>
...
// ... (Interface CartItem, ApiError, dan Constants STYLE tetap sama seperti sebelumnya) ...
// --- TIPE DATA ---
interface CartItem {
  id: number;
  bookId: number;
...
export default function CheckoutPage() {
  ...
  const { mutate: processCheckout, isPending: isProcessing } = useMutation({
    ...
    onSuccess: () => {
      // 1. Invalidate query agar data terupdate di background
      queryClient.invalidateQueries({ queryKey: ['my-cart'] });
      queryClient.invalidateQueries({ queryKey: ['my-loans'] });
      
      // 2. Tampilkan Pop-up Sukses alih-alih navigasi langsung
      setIsSuccess(true);
    },
    onError: (error: ApiError) => {
      toast.error('Gagal Meminjam', { 
