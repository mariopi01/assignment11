...
//     category: { id: number; name: string; };
//     availableCopies: number;
//     coverImage: string | null; // FIX: Tambahkan properti coverImage yang hilang
// }
...
//     // EFFECT BARU: Reset page ke 1 saat searchTerm, category, atau rating berubah (Hanya jika page > 1)
//     useEffect(() => {
//         if (page !== 1) {
//             setPage(1);
//         }
//         // Dependency array hanya bergantung pada searchTerm, selectedCategory, selectedRating
...
interface BookItem extends Omit<Book, 'author' | 'category'> {
    ...
    category: { id: number; name: string; };
    availableCopies: number;
    coverImage: string | null; // FIX: Tambahkan properti coverImage
}
...
export default function CategoryPage() {
    ...
    // Data Accumulation Logic (Menghilangkan cascading renders)
    // eslint-disable-next-line react-hooks/exhaustive-deps, react-hooks/set-state-in-effect
    useEffect(() => {
        if (booksResponse?.books) {
            setAllBooks(prevBooks => { 
