...
import { useState, useEffect } from 'react'; // import React is still included here for safety, though often not needed in newer React/TS configurations.
import { useParams, Link } from 'react-router-dom';
import { useQuery, useMutation, useQueryClient } from '@tanstack/react-query';
import { toast } from 'sonner';
import apiClient from '@/api';
...
import { Alert, AlertDescription, AlertTitle } from '@/components/ui/alert';
import { TriangleAlert, Loader2, Star, Share2, Heart, FileText, Users, BookOpen } from 'lucide-react'; 
...
const ReviewCard = ({ review }: { review: ReviewItem }) => {
    ...
    return (
        <Card className="flex flex-col h-full p-4 space-y-4 shadow-sm" 
            ...
            {/* Review Comment */}
            <CardContent className="p-0 text-start overflow-y-auto">
...
export default function BookDetailPage() {
  ...
  // FIX: Manual accumulation logic (Modified to resolve set-state-in-effect warning)
  useEffect(() => {
    if (reviewsResponse) {
    ...
    // FIX: Only depend on reviewsResponse (the fetched data object)
    // The totalReviews count and page logic are now handled inside the setAllReviews callback.
  }, [reviewsResponse]); 
  
  // FIX: Access pagination info
        ...
        <div className="grow max-w-full lg:max-w-[760px] space-y-4 text-left">
            ...
            <div className="flex items-center space-x-4 pt-6">
                ...
                >
                 ...
                 {/* Favorite Icon (Tambahan untuk melengkapi layout) */}
                <Button 
                    variant="outline"
                    className="rounded-full flex items-center justify-center p-0"
                    style={{ width: '44px', height: '44px', borderColor: BUTTON_OUTLINE, padding: '12px 16px' }}
